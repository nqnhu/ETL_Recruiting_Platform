...
def generate_fake_data(n_records, session, user, password, host, db_name):
...
status = "ON"
while status == "ON":
    generate_fake_data(n_records=random.randint(1, 20), session=session, user=user, password=password, host=host, db_name=db_name)
    time.sleep(10)